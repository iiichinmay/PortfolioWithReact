...
const Faq = () => {
  ...
  return (
    <div className="bg-black text-white min-h-screen" id="faq">
      <div>
        <h1 className="text-center text-4xl font-Cinzel mt-2 font-bold">
          <u>FAQ</u>
        </h1>

        <div className="border border-white p-5 flex flex-col items-center justify-center">
          {data.map((item, index) => (
            <div key={index} className="text-lg flex flex-col items-center mb-4 border border-white w-[60rem]  ">
              <h1 className="text-yellow-300 mb-2 font-SGummy cursor-pointer"
                onClick={() => toggleAccordion(index)}> {item.question} </h1>
                {activeIndex === index && ( <p className="font-SGummy w-[60rem] px-8 py-4 text-center"> {item.answer} </p> )}
            </div>
          ))}
        </div>
